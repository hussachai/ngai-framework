...
public class EntityUtil
{
	...
	/**
	 * Return the proxified class for passed class. If the passed class is not
	 * CGLIB enhanced, then it returns that same class.
	 * 
	 * Thanks Remi!
	 * 
	...
	/**
	 * Checks the fields and methods of <code>clazz</code> to
	 * determine the primary key's type. Results are cached to
	 * improve performance.
	 * 
	...
	public static Class<?> getIdType(Class<? extends Object> clazz)
	{
		...
		if(idType == Serializable.class){
			idType = getIdFromParameterizedSuperClass(clazz);
		}
		if (idType != null)
	...
	/**
	 * Looks for an Id annotation on the fields of <code>clazz</code> to
	 * figure out the primary key's type.
	 * 
	 * @param clazz the class to examine
	 * @return the primary key's type or null
	 */
	...
	/**
	 * Looks for an Id annotation on the methods of <code>clazz</code> to
	 * figure out the primary key's type.
	 * 
	 * @param clazz the class to examine
	 * @return the primary key's type or null
	...
	public static Class<?> getIdFromParameterizedSuperClass(Class<? extends Object> clazz){
		ParameterizedType ptype = ((ParameterizedType)clazz.getGenericSuperclass());
//		if(IAuditable.class.isAssignableFrom(clazz)){
