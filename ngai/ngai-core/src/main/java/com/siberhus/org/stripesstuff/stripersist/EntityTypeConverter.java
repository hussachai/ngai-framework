...
/**
 * <p>EntityTypeConverter retrieves an object of the specified target type by converting
 * the value parameter into the primary key type for the class and calling
 * {@link EntityManager#find(Class, Object)}.</p>
 * 
 * <p>{@link EntityFormatter} performs the reverse operation.</p>
 * 
 * @author Aaron Porter
 * @see EntityFormatter
 *
...
public class EntityTypeConverter implements TypeConverter<Object>
{
	...
	/**
	 * Converts <code>value</code> into the primary key's type for <code>targetType</code>
	 * and calls {@link EntityManager#find(Class, Object)}.
	 * 
	 * @param value the value to be converted
	 * @param targetType the type to return
	 * @param errors a collection of errors that occurred during validation and conversion
	...
	public Object convert(String value, Class<? extends Object> targetType, Collection<ValidationError> errors)
	{
		if (targetType.getAnnotation(Entity.class) == null &&
			targetType.getAnnotation(MappedSuperclass.class) == null)
		{
		...
		else
		{
			...
			try
			{
				TypeConverter<?> converter = StripesFilter.getConfiguration().getTypeConverterFactory().getTypeConverter(idType, locale);
			...
			{
				errors.add(new SimpleError("Could not find an instance of ", targetType.getName(), " with primary key ", id));
			}
