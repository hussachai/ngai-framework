			...
			<div class="span-workarea last">
				<display:table name="${actionBean.paginatedList}" uid="item"
					...
					<display:column titleKey="Action">
						<a href="#" onclick="UI.showDialog(this,{target:'${actionBeanPath}/view?layout=minimum&id=${item.id}&model.roleId=${model.roleId }&roleName=${actionBean.roleName}'});">view</a>
						&nbsp;|&nbsp;
						<s:link beanclass="${actionBeanClass}" event="edit" >edit
							<s:param name="model" value="${item.id}"></s:param>
							<s:param name="model.roleId" value="${model.roleId}"></s:param>
							<s:param name="roleName" value="${actionBean.roleName}"></s:param>
						</s:link>
						&nbsp;|&nbsp;
